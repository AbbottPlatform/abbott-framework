...
// migrated:
  botkit.receiveMessage = function (bot, message) {
    ...
    botkit.middleware.receive.run(bot, message, function (err, bot, message) {
      if (err) {
        ...
        botkit.logger.debug('RECEIVED MESSAGE');
  ...
  function Conversation(task, message) {
    ...
    this.handle = function (message) {
      ...
      botkit.logger.debug('HANDLING MESSAGE IN CONVO', message);
    ...
    this.on = function (event, cb) {
      botkit.logger.debug('Setting up a handler for', event);
      ...
      var makeChange = function () {
        if (!that.hasThread(that.next_thread)) {
          if (that.next_thread == 'default') {
            ...
            botkit.logger.debug('WARN: gotoThread() to an invalid thread!', thread);
    ...
    this.stop = function (status) {
      ...
      botkit.logger.debug('Conversation is over with status ' + this.status);
    ...
    this.onTimeout = function (handler) {
      if (typeof (handler) == 'function') {
        ...
        botkit.logger.debug('Invalid timeout function passed to onTimeout');
      }
    };
            ...
            if (typeof (this.messages[0].timestamp) == 'undefined' ||
              ...
              if (message) {
                ...
                this.task.bot.reply(this.source_message, outbound, function (err, sent_message) {
                  if (err) {
                    botkit.logger.error('An error occurred while sending a message: ', err);
    ...
    botkit.logger.debug('CREATED A CONVO FOR', this.source_message.user, this.source_message.channel);
  ...
  function Task(bot, message, botkit) {
    ...
    this.startConversation = function (message) {
      ...
      botkit.logger.debug('>   [Start] ', convo.id, ' Conversation with ', message.user, 'in', message.channel);
    ...
    this.conversationEnded = function (convo) {
      botkit.logger.debug('>   [End] ', convo.id, ' Conversation with ',
    ...
    this.taskEnded = function () {
      botkit.logger.debug('[End] ', this.id, ' Task for ',
  ...
  botkit.storage = {
    teams: {
      ...
      save: function (team, cb) {
        botkit.logger.warn('Warning: using temporary storage. Data will be lost when process restarts.');
        if (team.id) {
      ...
      if (message.text) {
        ...
        if (typeof (tests[t]) == 'string') {
          try {
            ...
            botkit.logger.error('Error in regular expression: ' + tests[t] + ': ' + err);
            return false;
          }
          if (!test) {
            return false;
      ...
      (function (keywords, test_function) {
        botkit.on(events[e], function (bot, message) {
          if (test_function && test_function(keywords, message)) {
            botkit.logger.debug('I HEARD', keywords);
...
// migrated:
  ...
  botkit.spawn = function (config, cb) {
    ...
    worker.say = function (message, cb) {
      ...
      botkit.middleware.send.run(worker, message, function (err, worker, message) {
        if (err) {
          botkit.logger.error('An error occured in the send middleware:: ' + err);
        } else {
          botkit.middleware.format.run(worker, message, platform_message, function (err, worker, message, platform_message) {
            if (err) {
              botkit.logger.error('An error occured in the format middleware: ' + err);
            } else {
    ...
    botkit.middleware.spawn.run(worker, function (err, worker) {
      if (err) {
        botkit.logger.error('Error in middlware.spawn.run: ' + err);
      } else {
  ...
  botkit.startTask = function (bot, message, cb) {
    ...
    task.id = botkit.taskCount++;
    botkit.logger.debug('[Start] ', task.id, ' Task for ', message.user, 'in', message.channel);
  ...
  botkit.setupWebserver = function (port, cb) {
    ...
    var server = botkit.webserver.listen(
      ...
      function () {
        botkit.logger.info('** Starting webserver on port ' +
          botkit.config.port);
        if (cb) {
  ...
  botkit.worker = function (botkit, config) {
    ...
    this.say = function (message, cb) {
      botkit.logger.debug('SAY:', message);
    };
    ...
    this.reply = function (src, resp) {
      botkit.logger.debug('REPLY:', resp);
    };


    this.findConversation = function (message, cb) {
      botkit.logger.debug('DEFAULT FIND CONVO');
      cb(null);
    };
  };
  ...
  if (configuration.logger) {
    ...
    botkit.logger = logging('abbott-framework:botkit:CoreBot');
  }

  // botkit.log = function () {
  //   botkit.logger.info.apply(botkit.log, arguments);
  // };
  ...
  // botkit.debug = botkit.logger.debug;
  ...
  if (configuration.storage) {
    if (
      ...
      botkit.logger.debug('** Using custom storage system.');
  ...
  //   botkit.logger.debug('** Using simple storage. Saving data to ' + configuration.json_file_store);
  //   botkit.storage = simple_storage({
  //     path: configuration.json_file_store
  //   });
  } else {
    botkit.logger.debug('** No persistent storage method specified! Data may be lost when process shuts down.');
  }
