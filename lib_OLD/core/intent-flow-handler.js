...
module.exports = class IntentFlowHandler {
  ...
  setContext(name) {
    return this.controller.contextManager.setContext(this.getContextSessionKey(), name);
  }

  getContext(name) {
    return this.controller.contextManager.getContext(this.getContextSessionKey(), name);
  }  
  ...
  _buildTextResponsePromises(responsePromises, payloadTargets, convo) {
  ...
  _buildCustomResponsePromises(responsePromises, payloadTargets, convo) {
    var payloadApis = [];
  ...
  _buildResponsePromises(convo) {
    ...
    this._buildTextResponsePromises(responsePromises, textMessages, convo);

    this._buildCustomResponsePromises(responsePromises, customPayloads, convo);
  ...
  _continueConversation(convo) {
    ...
    if (this.responseActionIncomplete) {
      ...
      let responsePromises = this._buildResponsePromises(convo);
