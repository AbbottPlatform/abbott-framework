...
function Slackbot(configuration) {
  ...
  configuration.logger = logging('abbott-framework:botkit:slack');
  ...
  // spawn!
  slack_botkit.defineBot(require(__dirname + '/worker.js'));
  ...
  slack_botkit.configureSlackApp = function (slack_app_config, cb) {
    ...
    slack_botkit.logger.info('** Configuring app as a Slack App!');
  ...
  slack_botkit.createHomepageEndpoint = function (webserver) {
    ...
    slack_botkit.logger.info('** Serving app landing page at : http://' +
  ...
  slack_botkit.handleWebhookPayload = function (req, res) {
    ...
    if (req.body.type === 'url_verification') {
      slack_botkit.logger.debug('Received url handshake');
    ...
    slack_botkit.findAppropriateTeam(payload, function (err, team) {
      if (err) {
        slack_botkit.logger.error('Could not load team while processing webhook: ', err);
        return;
      } else if (!team) {
        ...
        if (payload.type === 'event_callback') {
          ...
          if (!team.bot) {
            slack_botkit.logger.error('No bot identity found.');
            return;
          }
  ...
  slack_botkit.middleware.normalize.use(function handleEventsAPI(bot, message, next) {
    ...
    if (message.type == 'event_callback') {
      ...
      if (bot.identity == undefined || bot.identity.id == null) {
        slack_botkit.logger.error('Could not identify bot');
        return;
  ...
  slack_botkit.createOauthEndpoints = function (webserver, callback) {
    ...
    slack_botkit.logger.log('** Serving login URL: http://' +
      slack_botkit.config.hostname + ':' + slack_botkit.config.port + '/slack/login');
    ...
    var call_api = function (command, options, cb) {
      ...
      slack_botkit.logger.info('** API CALL: ' + api_root + '/api/' + command);
      request.post(api_root + '/api/' + command, function (error, response, body) {
        slack_botkit.logger.debug('Got response', error, body);
    ...
    webserver.get('/slack/login', function (req, res) {
      res.redirect(slack_botkit.getAuthorizeURL());
    });

    slack_botkit.logger.info('** Serving oauth return endpoint: http://' +
      slack_botkit.config.hostname + ':' + slack_botkit.config.port + '/slack/oauth');

    webserver.get('/slack/oauth', function (req, res) {
            ...
            if (err) {
              ...
              slack_botkit.findTeamById(identity.team_id, function (err, team) {
                ...
                slack_botkit.saveTeam(team, function (err, id) {
                  if (err) {
                    slack_botkit.logger.error('An error occurred while saving a team: ', err);
                    if (callback) {
                      callback(err, req, res);
                    } else {
                      res.status(500).send(err);
