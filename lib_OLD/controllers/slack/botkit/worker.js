...
const slackWebApi = require(__dirname + '/slack_web_api.js');
...
module.exports = function (botkit, config) {
  ...
  bot.sendWebhook = function (options, cb) {
    if (!bot.config.incoming_webhook || !bot.config.incoming_webhook.url) {
      botkit.logger.debug('CANNOT SEND WEBHOOK!!');
    ...
    botkit.middleware.send.run(bot, options, function (err, bot, options) {
      request.post(bot.config.incoming_webhook.url, function (err, res, body) {
        if (err) {
          botkit.logger.error('WEBHOOK ERROR', err);
          return cb && cb(err);
        }
  ...
  function reconnect(err) {
    ...
    return back.backoff(function (fail) {
      if (fail) {
        botkit.logger.error('** BOT ID:', bot.identity.name, '...reconnect failed after #' +
          back.settings.attempt + ' attempts and ' + back.settings.timeout + 'ms');
        botkit.trigger('rtm_reconnect_failed', [bot, err]);
        return;
      }

      botkit.logger.warn('** BOT ID:', bot.identity.name, '...reconnect attempt #' +
  ...
  bot.startRTM = function (cb) {
    ...
    bot.api.rtm.connect({}, function (err, res) {
      ...
      if (destroyed) {
        botkit.logger.warn('Ignoring rtm.start response, bot was destroyed');
        return cb('Ignoring rtm.start response, bot was destroyed');
      }

      botkit.logger.warn('** BOT ID:', bot.identity.name, '...attempting to connect to RTM!');
      ...
      bot.rtm.on('open', function () {
        botkit.logger.debug('RTM websocket opened');
        ...
        bot.rtm.on('message', function (data, flags) {
          ...
          try {
            ...
            console.logger.error('** RECEIVED BAD JSON FROM SLACK', err);
          }
          /**
        ...
        if (cb) cb(null, bot, res);
      });

      bot.rtm.on('error', function (err) {
        botkit.logger.error('RTM websocket error!', err);
        if (pingTimeoutId) {
          clearTimeout(pingTimeoutId);
        }
        botkit.trigger('rtm_close', [bot, err]);
      ...
      bot.rtm.on('close', function (code, message) {
        botkit.logger.debug('RTM close event: ' + code + ' : ' + message);
        if (pingTimeoutId) {
          clearTimeout(pingTimeoutId);
        }
         ...
         */
        if (code === 1006) {
          botkit.logger.warn('Abnormal websocket close event, attempting to reconnect');
          reconnect();
        }
      });
    });
  ...
  bot.identifyBot = function (cb) {
    ...
    if (bot.identity) {
       ...
       */
      if (cb) cb('Identity Unknown: Not using RTM api');
      return null;
    }
  };
  ...
  bot.identifyTeam = function (cb) {
    if (bot.team_info) {
      if (cb) cb(null, bot.team_info.id);
  ...
  bot.sendEphemeral = function (message, cb) {
    botkit.logger.debug('SAY EPHEMERAL', message);
  ...
  */
  bot.replyAcknowledge = function (cb) {
    if (!bot.res) {
      if (cb) cb(new Error('No web response object found'));
    } else {
      bot.res.end();
    ...
    if (!src.response_url) {
      ...
      botkit.middleware.send.run(bot, msg, function (err, bot, msg) {
        ...
        request(requestOptions, function (err, resp, body) {
           ...
           */
          if (err) {
            botkit.logger.error('Error sending slash command response:', err);
            if (cb) cb(err);
          } else {
            if (cb) cb();
          }
    ...
    if (!src.response_url) {
      ...
      botkit.middleware.send.run(bot, msg, function (err, bot, msg) {
        ...
        request(requestOptions, function (err, resp, body) {
           ...
           */
          if (err) {
            botkit.logger.error('Error sending interactive message response:', err);
            if (cb) cb(err);
          } else {
            if (cb) cb();
          }
  ...
  bot.findConversation = function (message, cb) {
    botkit.logger.debug('CUSTOM FIND CONVO', message.user, message.channel, message.type);
  ...
  bot.getInstanceInfo = function (cb) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      bot.api.auth.test({}, function (err, identity) {
        ...
        instance.identity.name = identity.user;
        instance.identity.id = identity.user_id;
        instance.team.name = identity.team;
        instance.team.url = identity.url;
        instance.team.id = identity.team_id;
