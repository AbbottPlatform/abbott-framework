...
function Facebookbot(configuration) {
  ...
  configuration.logger = logging('abbott-framework:botkit:facebook');
  ...
  facebook_botkit.defineBot(function (botkit, config) {
    ...
    bot.send = function (message, cb) {
      ...
      request({
        ...
        function (err, res, body) {
          ...
          if (err) {
            botkit.logger.debug('WEBHOOK ERROR', err);
            return cb && cb(err);
          }

          if (body.error) {
            botkit.logger.debug('API ERROR', body.error);
            return cb && cb(body.error.message);
          }

          botkit.logger.debug('WEBHOOK SUCCESS', body);
    ...
    bot.findConversation = function (message, cb) {
      botkit.logger.debug('CUSTOM FIND CONVO', message.user, message.channel);
      for (var t = 0; t < botkit.tasks.length; t++) {
        for (var c = 0; c < botkit.tasks[t].convos.length; c++) {
          if (
            ...
            botkit.logger.debug('FOUND EXISTING CONVO!');
  ...
  var messenger_profile_api = {
    ...
    postAPI: function (message) {
      request.post('https://' + api_host + '/v2.6/me/messenger_profile?access_token=' + configuration.access_token,
        ...
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not configure messenger profile');
          } else {
            ...
            try {
              ...
              facebook_botkit.logger.info('ERROR in messenger profile API call: Could not parse JSON', err01, body);
            }

            if (results) {
              if (results.error) {
                facebook_botkit.logger.log('ERROR in messenger profile API call: ', results.error.message);
              } else {
                facebook_botkit.logger.debug('Successfully configured messenger profile', body);
              }
            }
          }
        });
      ...
      request.delete('https://' + api_host + '/v2.6/me/messenger_profile?access_token=' + configuration.access_token,
        ...
        function (err, res, body) {
          if (err) {
            ...
            facebook_botkit.logger.debug('Successfully configured messenger profile', message);
          }
        });
    },
    getAPI: function (fields, cb) {
      request.get('https://' + api_host + '/v2.6/me/messenger_profile?fields=' + fields + '&access_token=' + configuration.access_token,
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not get messenger profile');
            cb(err);
          } else {
            facebook_botkit.logger.debug('Successfully got messenger profile ', body);
            cb(null, body);
          }
        });
    },
    get_messenger_code: function (image_size, cb, ref) {
      ...
      request.post('https://' + api_host + '/v2.6/me/messenger_codes?access_token=' + configuration.access_token,
        ...
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not configure get messenger code');
            cb(err);
          } else {
            ...
            try {
              ...
              facebook_botkit.logger.info('ERROR in messenger code API call: Could not parse JSON', err01, body);
              cb(err01);
            }

            if (results) {
              if (results.error) {
                facebook_botkit.logger.info('ERROR in messenger code API call: ', results.error.message);
                cb(results.error);
              } else {
                var uri = results.uri;
                facebook_botkit.logger.info('Successfully got messenger code', uri);
                cb(null, uri);
              }
            }
          }
  ...
  var attachment_upload_api = {
    upload: function (attachment, cb) {
      ...
      request.post('https://' + api_host + '/v2.6/me/message_attachments?access_token=' + configuration.access_token,
        ...
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not upload attachment');
            cb(err);
          } else {
            ...
            try {
              ...
              facebook_botkit.logger.info('ERROR in attachment upload API call: Could not parse JSON', err01, body);
              cb(err01);
            }

            if (results) {
              if (results.error) {
                facebook_botkit.logger.info('ERROR in attachment upload API call: ', results.error.message);
                cb(results.error);
              } else {
                var attachment_id = results.attachment_id;
                facebook_botkit.logger.info('Successfully got attachment id ', attachment_id);
  ...
  var tags = {
    get_all: function (cb) {
      request.get('https://' + api_host + '/v2.6/page_message_tags?access_token=' + configuration.access_token,
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not get tags list');
          } else {
            ...
            try {
              ...
              facebook_botkit.logger.info('ERROR in page message tags call: Could not parse JSON', err01, body);
            }

            if (results) {
              if (results.error) {
                facebook_botkit.logger.info('ERROR in page message tags: ', results.error.message);
              } else {
                facebook_botkit.logger.debug('Successfully call page message tags', body);
                cb(results);
              }
            }
          }
  ...
  var nlp = {
    ...
    postAPI: function (value, custom_token) {
      ...
      request.post(uri, {},
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not enable/disable build-in NLP');
          } else {
            ...
            try {
              ...
              facebook_botkit.logger.info('ERROR in build-in NLP API call: Could not parse JSON', err01, body);
            }

            if (results) {
              if (results.error) {
                facebook_botkit.logger.info('ERROR in build-in API call: ', results.error.message);
              } else {
                facebook_botkit.logger.debug('Successfully enable/disable build-in NLP', body);
              }
            }
          }
        });
  ...
  var user_profile = function (uid, fields, cb) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      request.get(uri, {},
        function (err, res, body) {
          if (err) {
            facebook_botkit.logger.info('Could not get user profile:', err);
            if (cb) {
              cb(err);
            } else {
              reject(err);
            ...
            try {
              ...
              facebook_botkit.logger.info('ERROR in user profile call: Could not parse JSON', err01, body);
              if (cb) {
                cb(err01);
              } else {
                reject(err01);
            ...
            if (results) {
              if (results.error) {
                facebook_botkit.logger.info('ERROR in user profile API call: ', results.error.message);
                if (cb) {
                  cb(results.error);
                } else {
  ...
  function abortOnValidationError(err, req, res, next) {
    if (err) {
      facebook_botkit.logger.info('** Invalid X-HUB signature on incoming request!');
      facebook_botkit.logger.debug('** X-HUB Validation Error:', err);
      res.status(400).send({
