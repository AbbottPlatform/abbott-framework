...
module.exports = function (abbottController) {
  abbottController.nlpProcessor.apiai.all(function (message, resp, bot) {
    ...
    if (abbottController.chatAnalytics) {
      ...
      if ((resp.result.action !== 'input.unknown') && (resp.result.metadata.intentName)) {
        intentName = resp.result.metadata.intentName;
      }

      abbottController.chatAnalytics.sendUserMessageAnayltics(message.text, message.user, intentName);
    }
